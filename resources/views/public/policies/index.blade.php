...
<main class="max-w-250 p-10 flex flex-col gap-10 mx-auto text-neutral-700">
  ...
  <header class="">
    ...
    <h1 class="text-4xl font-semibold py-3 text-center">Política de Privacidad de Navi Natubellez</h1>

    <p class="text-xl text-center text-gray-500">Comprometidos con la transparencia y protección de tus datos</p>
  ...
  <section class="grid grid-cols-1 gap-7 lg:grid-cols-3 ">
    ...
    <article class="p-5 flex items-center gap-4 shadow-lg rounded-lg border border-neutral-200">
      <div class="p-3 rounded-xl bg-pink-300/30 text-pink-500">
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-world-map">
          <path stroke="none" d="M0 0h24v24H0z" fill="none" />
  ...
  <section class="px-12 pb-12 shadow-xl rounded-2xl">
    ...
    <p class="text whitespace-pre-line text-lg font-medium pb-10">
      {{ $policy->description }}
    </p>

    <img class="w-full object-cover rounded-xl"
    src="{{ asset('storage/' . $policy->image) }}" alt="{{ $policy->title }}">
    ...
    <div class="p-5 mt-10 text-center">
      ...
      <p class="text-lg font-medium">Contáctanos en <span class="text-pink-400">{{ $siteInfo->correo }}</span></p>

    </div>

  </section>

  <p class="text-center">Úlitma actualizacion el {{ $policy->updated_at->format('d \d\e F \d\e Y') }}</p>
