<aside class="flex flex-col gap-4 max-w-55 shrink-0">

  <article class="p-5 rounded-lg border-1.5 border-neutral-200 flex-1 min-h-0 overflow-y-auto">
  ...
  <article class="p-5 rounded-lg border-1.5 border-neutral-200">
    ...
    <h1 class="text-xl font-semibold text-rose-400 pb-4">Precios</h1>
    ...
    <ul class="flex flex-col gap-4 font-medium xl:gap-5">
      ...
      <li class="{{ request('price_range') == '5-50' ? 'text-pink-400' : 'text-neutral-400 hover:text-pink-400' }}">
        <a href="{{ route('products', array_merge(request()->except('page'), ['price_range' => '5-50' ])) }}">
          S/5 - S/50
        </a>
      </li>

      <li class="{{ request('price_range') == '50-100' ? 'text-pink-400' : 'text-neutral-400 hover:text-pink-400' }}">
        <a href="{{ route('products', array_merge(request()->except('page'), ['price_range' => '50-100' ])) }}">
          S/50 - S/100
        </a>
      </li>

      <li class="{{ request('price_range') == '100-150' ? 'text-pink-400' : 'text-neutral-400 hover:text-pink-400' }}">
        <a href="{{ route('products', array_merge(request()->except('page'), ['price_range' => '100-150' ])) }}">
          S/100 - S/150
        </a>
      </li>

      <li class="{{ request('price_range') == '150-200' ? 'text-pink-400' : 'text-neutral-400 hover:text-pink-400' }}">
