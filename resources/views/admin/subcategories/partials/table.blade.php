        ...
        <tr>
            ...
            <td class="px-3 py-2">
                <div class="flex justify-center items-center gap-2">
                    <button class="px-3 py-1 text-yellow-500 rounded-md text-sm hover:text-yellow-700 transition-all" @click="openEdit({id: {{ $subcategory->id }}, name:@js($subcategory->name), category_id:{{ $subcategory->category_id }} })">
                    ...
                    <form action="{{ route('admin.subcategories.destroy', $subcategory) }}" method="POST" style="display:inline">
                        @csrf
                        @method('DELETE')
                        <button class="px-3 py-1 text-red-600 rounded-md text-sm hover:text-red-800 transition-all" type="submit" onclick="return confirm('¿Desea eliminar esta subcategoría?')">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
